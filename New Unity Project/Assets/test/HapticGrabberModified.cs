...
public class HapticGrabberModified : MonoBehaviour 
{
	...
	void Start () 
	{
		if (hapticDevice == null)
		{
			...
			HapticPluginModified[] HPs = (HapticPluginModified[])Object.FindObjectsOfType(typeof(HapticPluginModified));
			foreach (HapticPluginModified HP in HPs)
			{
		...
		if ( physicsToggleStyle != PhysicsToggleStyle.none)
			hapticDevice.GetComponent<HapticPluginModified>().PhysicsManipulationEnabled = false;
	...
	void FixedUpdate () 
	{
		bool newButtonStatus = hapticDevice.GetComponent<HapticPluginModified>().Buttons [buttonID] == 1;
		...
		if( grabbing && physicsToggleStyle != PhysicsToggleStyle.none)
			hapticDevice.GetComponent<HapticPluginModified>().PhysicsManipulationEnabled = true;
		if (!grabbing && physicsToggleStyle == PhysicsToggleStyle.onGrab)
			hapticDevice.GetComponent<HapticPluginModified>().PhysicsManipulationEnabled = false;

		/*
		if (grabbing)
			hapticDevice.GetComponent<HapticPluginModified>().shapesEnabled = false;
		else
			hapticDevice.GetComponent<HapticPluginModified>().shapesEnabled = true;
			*/
	...
	private void hapticTouchEvent( bool isTouch )
	{
		...
		{
			hapticDevice.GetComponent<HapticPluginModified>().PhysicsManipulationEnabled = isTouch;
	...
	void grab()
	{
		...
		if (touchedObject == null) // No Unity Collision? 
		{
			...
			touchedObject = hapticDevice.GetComponent<HapticPluginModified>().touching;
		}
